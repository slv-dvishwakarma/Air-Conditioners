...
export const Accordian: React.FC<AccordianProps> = ({ label, bank, button }) => {
      ...
      <div className='border mt-5 rounded-xl border-solid border-[#BEBEBE]'>
        {bank.map((items, index) => (
          <div key={index} className={`accordion-item px-[15px] py-3  ${index === 0 ? "border-b-[0.5px] border-b-[#BEBEBE] border-solid" : ""}`}>
            ...
            {activeIndex === index &&
              <AccountForm checkbox={items.checkbox} account={items.account} ifsc={items.ifsc} button={button} upi={items.upi} />
            }
          </div>
        ))}
      </div>
