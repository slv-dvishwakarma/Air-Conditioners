...
export const BreadCrumbs: React.FC<BreadCrumbProps> = ({ breadcrumb, delivery, compressor, energy, coil, appliances, title, price, brand, color }) => {
      ...
      <div className='border-b-[#BBBFBF] border-b border-solid px-0 py-5 flex justify-between'>
        <div className='breadcrumb '>
          {breadcrumb.map((item, index) => (
            <span key={index} className={`${index === breadcrumb.length - 1 ? "font-semibold" : "font-normal"} text-[#000000b8] xl:text-[15px] lg:text-[15px] md:text-[15px] text-[13px]`}>
