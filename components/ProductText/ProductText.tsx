...
export const ProductText: React.FC<ProductTextProps> = ({ label, name, control, errors, placeholder, required }) => {
    return (
        <>
            <Controller
                name={name}
                ...
                render={({ field: { onChange, value } }) => (
                    <div>
                        ...
                        <span className='input-border flex items-center border rounded-md border-solid border-[#BEBEBE] mt-[8px]'>
                            <input
                                type="text"
                                className="form-control w-full border-none rounded-md focus:outline-none focus:shadow-none shadow-none h-[40px] placeholder:text-[#9D9D9D] text-[14px] px-3"
