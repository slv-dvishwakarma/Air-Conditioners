...
export const LoginPopup: React.FC<LoginPopupProps> = ({ email, password, button, password_protection, checkbox, handleClose  }) => {
    ...
    const onSubmit = (data: any) => {
        const fakeCredentials = [
            { email: 'admin@gmail.com', password: 'admin@123', username: 'admin', role: 'admin' },
            { email: 'user@gmail.com', password: 'user@123', username: 'user', role: 'user' },
        ];
                        ...
                        <div className='space-y-2'>
                            <span className='flex gap-3'>
                                <input
                                    ...
                                    onChange={onChange}
                                />
                                <label htmlFor={checkbox.name} className='text-sm'>{checkbox.label}</label>
                            </span>
                        </div>
                    )}
                />
                <div className='mt-[10px]'>
                    {errors[checkbox.name] && <span className="text-[red] text-sm my-2">Please {checkbox.label}</span>}
                </div>
            </div>
            <button className='text-[15px] bg-accentColor rounded-[12px] text-white w-full px-5 py-2.5 mt-3'>{button}</button>
        </form>
    )
}
