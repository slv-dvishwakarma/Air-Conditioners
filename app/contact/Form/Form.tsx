...
export const Form: React.FC<EnquiryFormProps> = ({ title, enquiry_form, label, image, button }) => {
  return (
    <GridBox columns={2} gap={10} className='sm:grid py-[50px] items-center xl:space-y-0 lg:space-y-0 md:space-y-0 space-y-5'>
        ...
        <GridBox.GridItem columnMerge={1}>
        <EnquiryForm title={title} label={label} enquiry_form={enquiry_form} button={button}/>
        </GridBox.GridItem>
    </GridBox>
  )
}
