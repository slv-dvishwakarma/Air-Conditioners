...
export const ProductListing: React.FC<ProductListProps> = ({ product_list }) => {
    return (
        <GridBox columns={1} gap={5} className='pt-0 py-5 px-0 xl:space-y-0 lg:space-y-0 md:space-y-0 space-y-7'>
            {product_list.map((item, index) => {
                ...
                return (
                    <GridBox.GridItem key={index} columnMerge={1} colMargeDesktop={1} colMargeLaptop={1} colMargeTablet={1}>
                        <GridBox columns={4} gap={5} className='xl:space-y-0 lg:space-y-0 md:space-y-0 space-y-7'>
