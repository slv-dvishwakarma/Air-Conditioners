...
export const PriceFilter: React.FC<PriceFilterProps> = ({ price }) => {
    ...
    if (price.range[1] > price.maxPrice) {
      price.range[1] = price.maxPrice;
    }

    const [range, setRange] = useState<number[]>(price.range);
    const [fromValue, setFromValue] = useState<number | string>(price.range[0].toString());
    const [toValue, setToValue] = useState<number | string>(price.range[1].toString());

    useEffect(() => {
        setRange(price.range);
        setFromValue(price.range[0].toString());
        setToValue(price.range[1].toString());
    }, [price.range, price.maxPrice]);
