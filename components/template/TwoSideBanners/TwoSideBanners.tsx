...
export const TwoSideBanners: React.FC<BannersProps> = ({ banner }) => {
    return (
        <div className='banner pt-[20px]'>
            <div className="sm:grid xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-1 grid-cols-1  xl:gap-8 lg:gap-8 md:gap-3 gap-8 items-center space-y-5 xl:space-y-0 lg:space-y-0 md:space-y-0">
                {banner?.map((item, index) => (
                    <div key={index} className={`grid-item xl:px-[30px] lg:px-[20px] md:px-[30px] xl:py-[95px] lg:py-[50px] md:py-[50px] px-5 py-5 rounded-[20px]  col-span-${item.column_merge}`} style={{ backgroundImage: `url(${item.image})`, backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }}>
                        <div>
                            ...
                            <div className='xl:pt-[50px] lg:pt-[20px] md:pt-[20px] pt-[20px] xl:gap-5 lg:gap-5 md:gap-2 gap-5 flex'>
                                {item.button && item.button?.map((items, index) => (
                                    <Link className={` xl:px-[30px] lg:[30px] md:px-[6px] px-[10px] xl:py-3 lg:py-2 md:py-1 py-2 rounded-[10px] text-center  border border-solid border-accentColor xl:text-[15px] lg:text-[15px] md:text-sm text-sm ${index === 0 ? 'bg-accentColor text-white hover:bg-[transparent] hover:text-accentColor' : 'text-accentColor hover:bg-accentColor hover:text-white'}`} key={index} href={items.url}>{items.label}</Link>
                                ))}
                            </div>
                        </div>
                    </div>
