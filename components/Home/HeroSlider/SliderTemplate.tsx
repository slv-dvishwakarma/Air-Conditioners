...
export const SliderTemplate: React.FC<SliderProps> = ({ slider }) => {
  var settings = {
    infinite: true,
    speed: 500,
    arrows: false,
    ...
    <Slider className="custom-slick-slider" {...settings}>
      {slider.map((item, index) => (
        <div key={index} className='hero-slider' >
          <div className='bg-cover bg-no-repeat xl:py-[100px] lg:py-[100px] md:py-[50px] py-[50px] bg-left-top ' style={{ backgroundImage: `url(${item.image})` }}>
            <GridBox columns={3} desktop={3} laptop={3} tablet={2} className='px-[3%] sm:grid'>
              <GridBox.GridItem columnMerge={2} colMargeDesktop={2} colMargeLaptop={2} colMargeTablet={1}>
                <div></div>
              </GridBox.GridItem>
